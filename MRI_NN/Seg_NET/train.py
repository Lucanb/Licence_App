...
class SegNet(nn.Module):
    def __init__(self, num_classes=1, kernel_size=3, pool_size=(2, 2)):
        ...
        self.encoder = nn.ModuleList([
            nn.Sequential(
                ...
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
                nn.MaxPool2d(kernel_size=pool_size, stride=2, return_indices=True)
            ),
            nn.Sequential(
            ...
            nn.Sequential(
                ...
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True),
                nn.Conv2d(512, 512, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True),
                nn.Conv2d(512, 512, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(512),
        ...
        self.decoder = nn.ModuleList([
            nn.MaxUnpool2d(kernel_size=pool_size, stride=2),
            nn.Sequential(
                nn.ConvTranspose2d(512, 512, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(512, 512, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(512),
            ...
            nn.Sequential(
                ...
                nn.BatchNorm2d(512),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(512, 256, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True)
            ),
            nn.MaxUnpool2d(kernel_size=pool_size, stride=2),
            nn.Sequential(
                nn.ConvTranspose2d(256, 256, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(256, 256, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(256, 128, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(128),
                nn.ReLU(inplace=True)
            ),
            nn.MaxUnpool2d(kernel_size=pool_size, stride=2),
            nn.Sequential(
                nn.ConvTranspose2d(128, 128, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(128),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(128, 64, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True)
            ),
            nn.MaxUnpool2d(kernel_size=pool_size, stride=2),
            nn.Sequential(
                nn.ConvTranspose2d(64, 64, kernel_size=kernel_size, padding=1),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
                nn.ConvTranspose2d(64, num_classes, kernel_size=kernel_size, padding=1),
                nn.Sigmoid()
            )
        ])
